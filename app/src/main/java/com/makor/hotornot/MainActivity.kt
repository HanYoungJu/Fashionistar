...
class MainActivity : AppCompatActivity() {
    ...
    private fun showResult(result: Result) {
        var donot="I Can't Recognize"
        val line = 0.6
        if(result.confidence>line){
            textResult.text = result.result.toUpperCase()
        }
        else{
            textResult.text = donot
        }
    ...
    private fun getColorFromResult(result: String): Int {
        return if (result == "beauty") {
            resources.getColor(R.color.yellow)
        } else if(result == "man casual"){
            resources.getColor(R.color.sky)
        } else if(result == "man beach"){
            resources.getColor(R.color.pink)
        } else if(result == "woman beach"){
